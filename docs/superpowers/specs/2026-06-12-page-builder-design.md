...
```
src/lib/builder/
  types.ts              # WIDGET_TYPES union — nguồn sự thật danh sách widget
  config.client.tsx     # Puck Config cho editor (client): fields + defaultProps + render
                        # (fields CHỈ cần phía editor — server không dùng)
  config.server.tsx     # Config render-only cho <Render> công khai (RSC) —
                        # commerce widget query DB trực tiếp
  fields/               # custom fields: image (MediaPicker), richtext (Tiptap), color
...
- Giữ pipeline Tiptap hiện có: widget `richText` lưu `contentJson` (Tiptap JSON); HTML sinh **lúc render** qua `renderTiptapHtml()` (sanitized, server-side) — trang đã static-cache nên chi phí 1 lần/revalidate, bỏ được bước cache-on-save (`processBlocks`).
